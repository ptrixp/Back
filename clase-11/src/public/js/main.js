...
let user; //Acá en esta variable voy a almacenar el nombre del usuario. 
...
//keyup: escucha cada vez que teclea
chatBox.addEventListener("keyup", (event) => {
    if(event.key === "Enter") {
        if(chatBox.value.trim().length > 0) {
            ...
            chatBox.value = ""; //vacia los valores
        }
    }
})
...
socket.on("messagesLogs", data => {
    ...
    let messages = ""; //guardar mensajes vacios
